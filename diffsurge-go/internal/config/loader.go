...
func Load(cfgFile string) (*Config, error) {
	...
	if cfgFile != "" {
		...
		v.SetConfigName("diffsurge")
		v.SetConfigType("yaml")
		v.AddConfigPath(".")
		v.AddConfigPath("$HOME/.diffsurge")
		v.AddConfigPath("/etc/diffsurge")
	}

	v.SetEnvPrefix("DIFFSURGE")
...
// Priority: flags > SURGE_* env vars > DIFFSURGE_* env vars > .env file > defaults
func LoadCLI() *CLIConfig {
	...
	// DIFFSURGE_* fallback
	if cfg.APIKey == "" {
		if v := os.Getenv("DIFFSURGE_API_KEY"); v != "" {
			cfg.APIKey = v
		}
	}
	if cfg.ProjectID == "" {
		if v := os.Getenv("DIFFSURGE_PROJECT_ID"); v != "" {
			cfg.ProjectID = v
		}
	}
